...
class TestCLI(unittest.TestCase):
    ...
    def test_time_now(self):
        r = CLI.popen(['mrcs_time', '-n'])
        datetime = ISODatetime.construct_from_jdict(json.loads(r.stdout))
        self.assertIsNotNone(datetime)


    def test_time_set(self):
        CLI.popen(['mrcs_recorder', '-t', '-c', '-r', 1])

        r = CLI.popen(['mrcs_time', '-s', '-sr', '-ss', 4, '-sy', 1930, '-sm', 1, '-sd', 2, '-sh', 6])
        datetime = ISODatetime.construct_from_jdict(json.loads(r.stdout))
        self.assertIsNotNone(datetime)

        r = CLI.popen(['mrcs_messages', '-l', 100])
    ...
    def test_crontab(self):
        ...
        CLI.popen(['mrcs_publisher', '-t', 'CRN', '-b', 0, '-n', 3, '-m', message])

        r = CLI.popen(['mrcs_crontab', '-t', '-l'])
        jdict = json.loads(r.stdout)
